 ...
 *
 * Note: the documented title-search endpoint (`/v2/search`) is NOT exposed
 * here — as of 2026-09-07 it 500s on every query (confirmed directly against
 * api.unpaywall.org, independent of anything we send: different queries,
 * with/without is_oa, with/without a trailing slash all fail identically).
 * Unpaywall's own docs still describe the URL shape
 * (https://help.openalex.org/hc/en-us/articles/41193820492951, redirected
 * from support.unpaywall.org — Unpaywall folded into OpenAlex's 2025/2026
 * "Walden" rebuild), so it isn't formally retired, but it is non-functional
 * in production and there's no fix timeline. A tool that always errors costs
 * every caller a wasted call to discover that, so it was removed rather than
 * shipped broken (fleet #1328).
 */
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Unpaywall');
}
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  if (!email) {
    throw new Error(
      'Unpaywall requires a contact email (its polite-pool identifier, in place of an API key): pass it as the _email argument, e.g. _email=you@example.com.',
    );
  }
  switch (name) {
    case 'get_oa':
      return getOa(email, reqStr(args, 'doi', '"10.1038/s41586-023-06792-0"'));
    default:
...
interface OaRecord {
  ...
  // Unpaywall's z_authors entries carry a single pre-formatted display name
  // (raw_author_name), not separate given/family fields — confirmed live
  // 2026-09-07 against api.unpaywall.org (fleet #1328).
  z_authors?: { raw_author_name?: string }[];
}
...
function normalizeRecord(r: OaRecord) {
  return {
    ...
    authors: (r.z_authors ?? []).map((a) => a.raw_author_name).filter((n): n is string => Boolean(n)),
